...
let actions = {
  ...
  getLessons(){
    return (dispatch, getState) => {
      ...
      let {type, lessons: {loading, hasMore, offset, limit}} = getState().home;
      ...
      if (!loading && hasMore) {
        ...
        dispatch({
          ...
          payload: fetchLessons(type, offset, limit)
        });
      }
    }
  },
  refresh(){
    return (dispatch, getState) => {
      ...
      if (!loading) {
        ...
        dispatch({
          type: types.REFRESH_LESSONS_FINISH,
          payload: fetchLessons(type, 0, limit)
        });
      }
    }
  }
